						...
						out half out_temporal_AO )
{
		  ...
		  ( reproj_screenPos.y > 1.0 ) ) == false )
	{
		...
		half prev_sampledDepth = prev_AO_Depth_N.y;
		half prev_N = prev_AO_Depth_N.z;
		const half2 cOcclusionDepth = half2( aCurrAO, aVposZ );

		const half mvLenght = length( mv );

		half disocclusion = CalcDisocclusion( aSampledDepth, prev_sampledDepth, ( _AO_TemporalMotionSensibility * 20.0 + 1.0 ) * aVposZ );

		#if defined( DISOCCLUSION_NEIGHBOR )
		if( aUseMotionVectors == false )
		{
			prev_N = lerp( prev_N, min( prev_N , NeighborPrevN( aScreenPos, aVposZ ) ), saturate( _AO_TemporalMotionSensibility * 0.50 + 0.50 ) );
		}
		#endif
		...
		half4 depthWeight0123 = saturate( 1.0 / ( abs( LinearEyeToSampledDepth( d0123 ) - ( aSampledDepth ).xxxx ) * DEPTH_WEIGHTING + DEPTH_WEIGHTING_OFFSET ) );
		...
		#else
		half cMax = max( cOcclusionDepth, max( cTL, max( cTR, max( cBL, cBR ) ) ) );
		half cMin = min( cOcclusionDepth, min( cTL, min( cTR, min( cBL, cBR ) ) ) );
		#endif
		...
		half cMax = min( mu + Sigma * SIGMA_SCALE_MAX, 1.0 );
		#else
		half cMax = max( cOcclusionDepth, max( cT, max( cL, max( cR, cB ) ) ) );
		half cMin = min( cOcclusionDepth, min( cT, min( cL, min( cR, cB ) ) ) );
		#endif
		...
		half4 depthWeight4567 = saturate( 1.0 / ( abs( LinearEyeToSampledDepth( d4567 ) - ( aSampledDepth ).xxxx ) * DEPTH_WEIGHTING + DEPTH_WEIGHTING_OFFSET ) );
		...
		half cMax = min( mu + Sigma * SIGMA_SCALE_MAX, 1.0 );
		#else
		half cMax = max( cOcclusionDepth, max( cTL, max( cTR, max( cBL, max( cBR, max( cT, max( cL, max( cR, cB ) ) ) ) ) ) ) );
		half cMin = min( cOcclusionDepth, min( cTL, min( cTR, min( cBL, min( cBR, min( cT, min( cL, min( cR, cB ) ) ) ) ) ) ) );
		#endif
...
half4 TemporalFilter( const half2 aScreenPos, const half2 aOcclusionDepth, const bool aUseMotionVectors, out half outOcclusion )
{
	const half currAO = aOcclusionDepth.x;

	half out_ao, out_newN, out_temporal_ao;

	const half sampledDepth = LinearEyeToSampledDepth( aOcclusionDepth.y );
	...
	return half4( EncAO( out_temporal_ao ), EncDepth( sampledDepth ), out_newN );
}
