...
inline half3 FetchNormal( const half2 uv, const uint normalSource )
{
	...
	{
		const half3 c = FetchPosition0( uv ).xyz;
		const half3 r = FetchPosition0( uv + half2( +1.0 , 0.0 ) * _AO_Target_TexelSize.xy ).xyz;
		const half3 l = FetchPosition0( uv + half2( -1.0 , 0.0 ) * _AO_Target_TexelSize.xy ).xyz;
		const half3 t = FetchPosition0( uv + half2(  0.0, +1.0 ) * _AO_Target_TexelSize.xy ).xyz;
		const half3 b = FetchPosition0( uv + half2(  0.0, -1.0 ) * _AO_Target_TexelSize.xy ).xyz;
