Shader "ChillyRoom/Noise/PerlinNoise2D_Tileable"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 perlin2D_Tile(float2 uv) : SV_Target
			{
				...
				float s = uv.x;
				float t = uv.y;
				float f = _Scale*0.511165f;

				//dx / (2 * Math.PI) 圆的半径
				float nx = (float)(x1 + sin(s * 2 * UNITY_PI)  *r / (2 * UNITY_PI)) * f;
				float ny = (float)(y1 + cos(s * 2 * UNITY_PI)  *r / (2 * UNITY_PI)) * f;
				float nz = (float)(x1 + sin(t * 2 * UNITY_PI)  *r / (2 * UNITY_PI)) * f;
				float nw = (float)(y1 + cos(t * 2 * UNITY_PI)  *r / (2 * UNITY_PI)) * f;

				return perlin4D(float4(nx, ny, nz, nw));
			}

#ifdef _USEFBM
			float noise(float2 uv) {
				return perlin2D_Tile(uv).r;
			}
#endif
...
#else
				fixed4 col = perlin2D_Tile(i.globalTexcoord.xy);
#endif
			return col;
			}
			ENDCG
